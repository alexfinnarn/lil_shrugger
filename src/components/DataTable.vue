  ...
  <div class="panel panel-default">
    ...
    <div class="panel-body">
      <form id="search" class="row">
        <div class="form-group">
          ...
          <div class="col col-md-5" id="expression-search-buttons">
            <button
              v-if="!expressionFilter"
              class="btn btn-default"
              @click.prevent="expressionFilter = !expressionFilter"
              aria-label="Toggle Expression Search"
            >
            ...
            <button
              ...
              @click.prevent="expressionFilterSearch()"
              aria-label="Expression Search"
            >Expression Search</button>
            <button
              v-if="expressionFilter"
              class="btn btn-default"
              @click.prevent="cancelExpressionFilterSearch()"
              aria-label="Expression Search"
            >Reset</button>
            <div class="col pull-right" id="add-row" v-if="userAccessPerm('addRow')">
              <button
                @click="addRow()"
                type="button"
                class="btn btn-primary"
      ...
      <div class="table-responsive">
        ...
        <div v-show="showAdd || showEdit || showView" class="row col col-md-12">
          <div v-show="showAdd">
          ...
          <div v-show="showView">
            ...
            <ul class="nav nav-tabs" role="tablist">
              <li
                v-for="(comp, index) in rowViewComponents"
                role="presentation"
        ...
        <div class="show-more-buttons" v-if="resultCount > showRowCount">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      sortKey: this.tableOptions.defaultSortKey
        ? this.tableOptions.defaultSortKey
        : "",
      sortOrders,
      showAllRows: false,
      allChecked: false,
    ...
    filterKey: {
      ...
      set(value) {
        store.commit("setFilterKey", [
          { type: this.tableOptions.dataName, filterString: value }
        ]);
      }
    },
    resultCount() {
  ...
  methods: {
    ...
    rowViewListener(that, row, action) {
      ...
      let foundRow = [];
      if (action !== "showAdd") {
        foundRow = Object.values(that.gridData).filter(
          value => value.id === row.data.id
        );
      }
    ...
    cancelExpressionFilterSearch() {
      ...
      store.commit("setFilterKey", [
        { type: this.tableOptions.dataName, filterString: "" }
      ]);
